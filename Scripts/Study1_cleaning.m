...
[~, ~, rawTextData] = xlsread('dataSet1_EOD_event descriptions_fixed_duplicates removed.csv'); % import the data in its original format (i.e., both string and numerical)
numericData = rawTextData(2:end,[1:3 5]);
textData = rawTextData(2:end,4);
subjectIDlistText = unique(cell2mat(numericData(:,2)));

% emotion intensity ratings
rawRatingData = importdata('dataSet1_EOD_rating data.xlsx');
...
lengthText = cell2mat(numericData(:,4));
...
%% export data
newData = [numericData(:,1:3) textData];
newData = [{'Filename','PPID','Day','Text'}; newData]; % optional headers
xlswrite('dataSet1_EOD_event descriptions_fixed_duplicates removed_final N.csv',newData);

newData_30 = [numericData_30(:,1:3) textData_30];
newData_30 = [{'Filename','PPID','Day','Text'}; newData_30]; % optional headers
xlswrite('dataSet1_EOD_event descriptions_fixed_duplicates removed_final N_min 30.csv',newData_30);

newData_25 = [numericData_25(:,1:3) textData_25];
newData_25 = [{'Filename','PPID','Day','Text'}; newData_25]; % optional headers
xlswrite('dataSet1_EOD_event descriptions_fixed_duplicates removed_final N_min 25.csv',newData_25);

newData_20 = [numericData_20(:,1:3) textData_20];
newData_20 = [{'Filename','PPID','Day','Text'}; newData_20]; % optional headers
xlswrite('dataSet1_EOD_event descriptions_fixed_duplicates removed_final N_min 20.csv',newData_20);
...
%% set parameters
ngrams = 150; % set to number of n-grams extracted in MEH
components = 10; % set to number of themes extracted in PCA
generateFigures = 0; % set to 1 to generate participant-level figures

%% load and configure data file
rawData = importdata(['MEM Themes - Dataset - Scored via Relative Frequencies - Changepoints High_' num2str(ngrams) '_' num2str(components) '.csv']);
...
load(['dataSet1_granularity_affect_measures.mat']); % load matrix of variables per subject (measures)
measures(:,2:4) = measures(:,2:4)*-1; % invert zICC values to get intuitively scaled granularity 
...
for i_subject = 1:length(subjectIDlist)
    ...
    if generateFigures == 1
        ...
        ylim([0 1]);
        title(['Participant ' num2str(subjectIDlist(i_subject)) '; count ' num2str(totalCount(i_subject,1)) '; diversity ' num2str(giniCoefTheme(i_subject,1))]);
...
%% summary table
summary = [table2array([mdl1_S.Coefficients(end,1) mdl1_S.Coefficients(end,4)])...
...
if generateFigures == 1
    ...
    ratingData = importdata('dataSet1_EOD_rating data_by valence.xlsx');
    subjectIDlist2 = unique(ratingData.data(:,1)); % grab subject IDs from first column of data file
    wordList = ratingData.colheaders(2:end)';  % grab sampled words from top row of data file

    % generate matrix for high participant
    highPPdata = ratingData.data(find(ratingData.data(:,1)==highPP),2:end);
    ...
    lowPPdata = ratingData.data(find(ratingData.data(:,1)==lowPP),2:end);
    ...
    figure;
    bar2 = bar(propByTheme(find(subjectIDlist==highPP),:),'FaceColor',rgb('ForestGreen')); 
    xlabel('Theme');
    ylim([0 .8]);
    ...
    figure;
    bar3 = bar(propByTheme(find(subjectIDlist==lowPP),:),'FaceColor',rgb('MediumPurple')); 
    xlabel('Theme');
    ylim([0 .8]);
    ylabel('Proportion Texts');
    set(gca,'fontsize',14)

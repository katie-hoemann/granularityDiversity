...
[~, ~, rawTextData] = xlsread('dataSet2_diary entries_cleaned PPs_empties removed.xlsx'); % import the data in its original format (i.e., both string and numerical)
...
rawRatingData = importdata('dataSet2_ESM_rating data.xlsx');
...
newData = [{'Filename','PPID','Diary','Type','Text'}; newData]; % optional headers
xlswrite('dataSet2_diary entries_cleaned PPs_empties removed_final N.csv',newData);

newData_30 = [numericData_30(:,1:4) textData_30];
newData_30 = [{'Filename','PPID','Diary','Type','Text'}; newData_30]; % optional headers
xlswrite('dataSet2_diary entries_cleaned PPs_empties removed_final N_min 30.csv',newData_30);

newData_25 = [numericData_25(:,1:4) textData_25];
newData_25 = [{'Filename','PPID','Diary','Type','Text'}; newData_25]; % optional headers
xlswrite('dataSet2_diary entries_cleaned PPs_empties removed_final N_min 25.csv',newData_25);

newData_20 = [numericData_20(:,1:4) textData_20];
newData_20 = [{'Filename','PPID','Diary','Type','Text'}; newData_20]; % optional headers
xlswrite('dataSet2_diary entries_cleaned PPs_empties removed_final N_min 20.csv',newData_20);
